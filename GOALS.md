...
2. `python -m rag_eval demo` runs the same eval at `k=5` vs `k=1` (same BM25 retriever, only `k` changes) and automatically flags aggregate regressions. Measured: retrieval_hit 0.88 -> 0.72, groundedness 0.81 -> 0.79, f1 0.137 -> 0.123.
