...
def _add_generator_args(parser: argparse.ArgumentParser) -> None:
    parser.add_argument(
        "--generator",
        choices=["mock", "local", "api", "hf", "hfgen"],
        default="mock",
        help=(
            ...
            "no token). 'hfgen' runs a local HuggingFace generative instruct model "
            "on CPU (no server, no token)."
        ),
    )
    parser.add_argument(
        "--model",
...
def system_label(
    ...
    if generator_name == "hfgen":
        model_label = _sanitize_label(model or DEFAULT_GENERATIVE_MODEL)
        return f"hfgen-{model_label}-k{k}"
